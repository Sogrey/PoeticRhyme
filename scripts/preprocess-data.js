...
function saveJson(data, filename, maxSize = MAX_FILE_SIZE) {
  ...
  if (size <= maxSize) {
    ...
    console.log(`Saved: ${filename} (${(size / 1024).toFixed(2)} KB)`);
    return [{ path: filePath, size }];
  }

  console.log(
    `File ${filename} (${(size / 1024).toFixed(2)} KB) exceeds ${maxSize / 1024 / 1024}MB, splitting...`,
  );
  ...
  chunks.forEach((chunk, index) => {
    const chunkFileName = chunks.length > 1 ? `${baseName}.${index + 1}.json` : filename;
    const chunkPath = path.join(PUBLIC_DIR, chunkFileName);
    const chunkContent = JSON.stringify(chunk, null, 2);
    fs.writeFileSync(chunkPath, chunkContent, "utf-8");
    const chunkSize = Buffer.byteLength(chunkContent);
    console.log(`Saved: ${chunkFileName} (${(chunkSize / 1024).toFixed(2)} KB)`);
    savedPaths.push({ path: chunkPath, size: chunkSize });
  });
...
function processPoetryItem(item, dynasty, typePrefix) {
  const authorInfo = item.author || {};
  const authorNameRaw = typeof authorInfo === "string" ? authorInfo : authorInfo.name || "未知";
  const authorName = simplifyText(authorNameRaw);
  ...
  } else if (typeof item.paragraph === "string") {
    content = item.paragraph;
  }

  const contentSimplified = simplifyText(content);
  const titleSimplified = simplifyText(item.title || "无题");

  return {
    id: item.id || `${typePrefix}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    title: titleSimplified,
    content: contentSimplified,
    author: authorName,
    authorId: `author_${authorName.replace(/\s/g, "_")}`,
    dynasty: dynasty,
    tags: extractTags(item),
    rhythmic: simplifyText(item.rhythmic || ""),
    paragraphs: contentSimplified.split("\n"),
  };
}
...
function extractTags(item) {
  ...
  if (item.tags && Array.isArray(item.tags)) {
    tags.push(...item.tags.map((tag) => simplifyText(tag)));
  }
  if (item.rhythmic) {
    tags.push("词牌:" + simplifyText(item.rhythmic));
  }
  if (item.constellation) {
    tags.push(simplifyText(item.constellation));
  }
  if (item.type) {
    tags.push(simplifyText(item.type));
  }
  return [...new Set(tags)].filter(Boolean);
}

function processAuthor(authorData, dynasty) {
  const nameRaw = authorData.name || authorData.author || "未知";
  const name = simplifyText(nameRaw);
  return {
    id: `author_${name.replace(/\s/g, "_")}`,
    name: name,
    dynasty: dynasty,
    description: simplifyText(authorData.description || authorData.intro || ""),
    poetryCount: authorData.poetryCount || 0,
  };
}

function processTangshiFiles() {
  console.log("\n--- Processing Tang Poetry (全唐诗) ---");
  const tangDir = path.join(REPO_DIR, "全唐诗");

  const poems = [];
  const authors = new Map();

  const tangFiles = getJsonFiles(tangDir).filter(
    (f) => f.includes("poet.tang") || f.includes("唐诗三百首"),
  );
  ...
  for (const file of tangFiles) {
    ...
    if (data && Array.isArray(data)) {
      const processed = data.map((item) => processPoetryItem(item, "唐", "tang"));
      poems.push(...processed);
    }
  }

  const authorFile = path.join(tangDir, "authors.tang.json");
  const authorData = loadJson(authorFile);
  if (authorData && Array.isArray(authorData)) {
    authorData.forEach((item) => {
      const processed = processAuthor(item, "唐");
      if (!authors.has(processed.id)) {
        authors.set(processed.id, processed);
      }
    });
  }

  console.log(`Processed ${poems.length} Tang poems, ${authors.size} authors`);
  return { poems, authors };
}

function processSongciFiles() {
  console.log("\n--- Processing Song Ci (宋词) ---");
  const ciDir = path.join(REPO_DIR, "宋词");

  const poems = [];
  const authors = new Map();

  const ciFiles = getJsonFiles(ciDir).filter(
    (f) => f.includes("ci.song.") || f.includes("宋词三百首"),
  );

  console.log(`Found ${ciFiles.length} Song Ci files`);

  for (const file of ciFiles) {
    console.log(`Processing: ${path.basename(file)}`);
    const data = loadJson(file);
    if (data && Array.isArray(data)) {
      const processed = data.map((item) => processPoetryItem(item, "宋", "songci"));
      poems.push(...processed);

      data.forEach((item) => {
        if (item.author) {
          const authorName =
            typeof item.author === "string" ? item.author : item.author.name || "未知";
          const authorId = `author_${authorName.replace(/\s/g, "_")}`;
          if (!authors.has(authorId)) {
            authors.set(authorId, processAuthor({ name: authorName }, "宋"));
          }
        }
      });
    }
  ...
  console.log(`Processed ${poems.length} Song Ci, ${authors.size} authors`);
  return { poems, authors };
}

function processShijingFiles() {
  console.log("\n--- Processing Shi Jing (诗经) ---");
  const shijingFile = path.join(REPO_DIR, "诗经", "shijing.json");

  const poems = [];
  const data = loadJson(shijingFile);

  if (data && Array.isArray(data)) {
    console.log(`Processing ${data.length} Shi Jing poems`);
    const processed = data.map((item) => processPoetryItem(item, "先秦", "shijing"));
    poems.push(...processed);
  }

  console.log(`Processed ${poems.length} Shi Jing poems`);
  return { poems };
}

function processLunyuFiles() {
  console.log("\n--- Processing Lun Yu (论语) ---");
  const lunyuFile = path.join(REPO_DIR, "论语", "lunyu.json");

  const chapters = [];
  const data = loadJson(lunyuFile);

  if (data && Array.isArray(data)) {
    console.log(`Processing ${data.length} Lun Yu chapters`);
    data.forEach((item, index) => {
      const contentRaw = Array.isArray(item.paragraphs)
        ? item.paragraphs.join("\n")
        : item.content || "";
      const titleRaw = item.title || `论语·第${index + 1}篇`;
      chapters.push({
        id: `lunyu_${index}`,
        title: simplifyText(titleRaw),
        content: simplifyText(contentRaw),
        chapter: simplifyText(item.chapter || item.title || ""),
        paragraphs: simplifyText(contentRaw).split("\n"),
      });
    });
  }

  console.log(`Processed ${chapters.length} Lun Yu chapters`);
  return { chapters };
}

function buildIndexes(allPoems) {
  const authors = new Map();
  const dynasties = new Map();
  const tags = new Map();

  allPoems.forEach((poem) => {
    if (poem.authorId && !authors.has(poem.authorId)) {
      authors.set(poem.authorId, poem.author);
    }

    if (poem.dynasty && !dynasties.has(poem.dynasty)) {
      dynasties.set(poem.dynasty, true);
    }
  ...
  return {
    authors: Array.from(authors.entries()).map(([id, name]) => ({ id, name })),
...
async function preprocessData() {
  if (!fs.existsSync(REPO_DIR)) {
    console.error("Error: Repository not found. Please run 'npm run data:fetch' first.");
    process.exit(1);
  }
  ...
  const tangResult = processTangshiFiles();
  allPoems.push(...tangResult.poems);
  tangResult.authors.forEach((author, id) => allAuthors.set(id, author));

  const songciResult = processSongciFiles();
  allPoems.push(...songciResult.poems);
  songciResult.authors.forEach((author, id) => allAuthors.set(id, author));

  const shijingResult = processShijingFiles();
  allPoems.push(...shijingResult.poems);

  const lunyuResult = processLunyuFiles();

  const indexes = buildIndexes(allPoems);

  console.log("\n========================================");
  console.log("Saving processed data...");
  console.log("========================================");

  saveJson(allPoems, "poems.json");
  saveJson(Array.from(allAuthors.values()), "authors.json");
  saveJson(indexes, "indexes.json");
  saveJson(shijingResult.poems, "shijing.json");
  saveJson(lunyuResult.chapters, "lunyu.json");

  const stats = {
    totalPoems: allPoems.length,
    tangshiCount: tangResult.poems.length,
    songciCount: songciResult.poems.length,
    shijingCount: shijingResult.poems.length,
    lunyuCount: lunyuResult.chapters.length,
    authorsCount: allAuthors.size,
    dynasties: [...new Set(allPoems.map((p) => p.dynasty))],
    processedAt: new Date().toISOString(),
  };
  saveJson(stats, "stats.json");

  console.log("\n========================================");
  console.log("Data Preprocessing Summary");
  console.log("========================================");
  console.log(`Total poems: ${stats.totalPoems}`);
  console.log(`Tangshi: ${stats.tangshiCount}`);
  console.log(`Songci: ${stats.songciCount}`);
  console.log(`Shijing: ${stats.shijingCount}`);
  console.log(`Lunyu: ${stats.lunyuCount}`);
  console.log(`Authors: ${stats.authorsCount}`);
  console.log(`Dynasties: ${stats.dynasties.join(", ")}`);
  console.log("========================================\n");

  return stats;
}

if (process.argv[1] && process.argv[1].includes("preprocess-data.js")) {
  preprocessData().catch(console.error);
}

export { preprocessData, REPO_DIR, TMP_DIR, PUBLIC_DIR };
